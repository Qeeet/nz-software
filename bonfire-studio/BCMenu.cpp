                        ...
                        int nHeight, CBitmap &bmp, int nXSrc, int nYSrc)
{
	...
	CDC dcBmp; dcBmp.CreateCompatibleDC(drawdc);
	CBitmap* oldbmp1 = dcBmp.SelectObject(&bmp);

	// create a DC for the DIB
	CDC dcDib; dcDib.CreateCompatibleDC(drawdc);
	...
	void* pBits;
	CBitmap dib; dib.Attach(::CreateDIBSection(dcDib,&bi,DIB_RGB_COLORS,&pBits,NULL,0));
	CBitmap* oldbmp2 = dcDib.SelectObject(&dib);

	// copy the bitmap data to the DIB data
	dcDib.BitBlt(0,0,nWidth,nHeight,&dcBmp,nXSrc,nYSrc,SRCCOPY);

	// delete the bitmap dc
	dcBmp.SelectObject(oldbmp1);
	dcBmp.DeleteDC();

	// modify the DIB
	dcDib.SelectObject(oldbmp2);
	::CreateDisabledImage(dib);
	oldbmp2 = dcDib.SelectObject(&dib);

	// blt the DIB onto the destination DC
	drawdc->BitBlt(nXDest,nYDest,nWidth,nHeight,&dcDib,0,0,SRCCOPY);
