...
static const TCHAR *crlfs[] =	         //UPDATE
{
...
BOOL CCrystalTextBuffer::LoadFromFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bSourceExists /*= TRUE*/)
{
	ASSERT(! m_bInit);
	ASSERT(m_aLines.GetSize() == 0);
	...
	// ADDED
	BOOL bAskedSplitFileLines = !bSourceExists;

	// can't use GetFileTitle because of error C2712
	char str[256];
	char strTitle[256];
	char* pTitleBegin = strrchr(pszFileName,'\\');
	if (pTitleBegin)
		strcpy(strTitle,pTitleBegin + 1);
	...
	__try
	{
		...
		if (nCrlfStyle == CRLF_STYLE_AUTOMATIC)				//UPDATE
		{
		...
		ASSERT(nCrlfStyle >= 0 && nCrlfStyle <= 2);
		m_nCRLFMode = nCrlfStyle;
		const TCHAR *crlf = crlfs[nCrlfStyle];
		...
		// ADDED
		// keep track of < or > to break off the line
		DWORD dwSplitPos = 0;
		...
		while (dwBufPtr < dwCurSize)
		{
			...
			// ADDED
			if (nCurrentLength > LINE_MAX)
			{
				if (!bAskedSplitFileLines)
				{
					sprintf(str,"The file '%s' contains one or more line(s) with more than %d characters. To split these lines, press OK. Otherwise, press Cancel to stop loading the file.",
						strTitle,LINE_MAX);

					if (AfxMessageBox(str,MB_OKCANCEL | MB_ICONEXCLAMATION) == IDCANCEL)
						__leave;

					bAskedSplitFileLines = TRUE;
				}

				if (dwSplitPos > 0)
				{
					// cut off the line at the last < or > or space
					int nTillEnd = nCurrentLength - dwSplitPos;
					char *pcTempBuf = new char[nCurrentMax];
					memcpy(pcTempBuf, pcLineBuf + dwSplitPos, nTillEnd);
					pcLineBuf[dwSplitPos] = 0;
					InsertLine(A2T(pcLineBuf));
					delete pcLineBuf;
					pcLineBuf = pcTempBuf;
					nCurrentLength = nTillEnd;
				}
				else
				{
					// cut the line off at [LINE_MAX]th character
			...
			{
				// check for < or > or space to break the line off here
				// dont wanna check if its the character that puts us over the top
				if (c == '<')
					dwSplitPos = nCurrentLength - 1;
				else if (c == '>')
					dwSplitPos = nCurrentLength;
				else if (isspace(c))
					dwSplitPos = nCurrentLength;
			}
...
BOOL CCrystalTextBuffer::SaveToFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bClearModifiedFlag /*= TRUE*/)
{
	...
	__try
	{
		...
		ASSERT(nCrlfStyle >= 0 && nCrlfStyle <= 2);
		const TCHAR *pszCRLF = crlfs[nCrlfStyle];	 //UPDATE
		int nCRLFLength = lstrlen(pszCRLF);			 //UPDATE
...
BOOL CCrystalTextBuffer::GetActionDescription(int nAction, CString &desc)
{
	...
	case CE_ACTION_AUTOINDENT:
		...
		break;
	case CE_ACTION_TGL_COMMENT:
		bSuccess = desc.LoadString(IDS_EDITOP_TGL_COMMENT);
		break;
	}
