...
void CSizingControlBar::OnNcPaint()
{
    ...
    if (m_dwSCBStyle & SCBS_SHOWEDGES)
    {
		...
		{
			BOOL bDrawGrip = 
				((GetBarStyle() & CBRS_ALIGN_TOP) || 
				 (GetBarStyle() & CBRS_ALIGN_BOTTOM)) ==
				 (i % 2 != 0);

            if (bDrawGrip && GetEdgeRect(rcBar, GetEdgeHTCode(i), rcEdge))
			{
                /*mdc.Draw3dRect(rcEdge, ::GetSysColor(COLOR_BTNHIGHLIGHT),
                    ::GetSysColor(COLOR_BTNSHADOW));

				CPen pen; pen.CreatePen(PS_SOLID,1,(COLORREF)GetSysColor(COLOR_3DDKSHADOW));
				CPen* oldpen = mdc.SelectObject(&pen);

				if (GetBarStyle() & CBRS_ALIGN_RIGHT)
				{
					mdc.MoveTo(rcEdge.right,rcEdge.top);
					mdc.LineTo(rcEdge.right,rcEdge.bottom);
				}
				else if (GetBarStyle() & CBRS_ALIGN_BOTTOM)
				{
					mdc.MoveTo(rcEdge.left,rcEdge.bottom);
					mdc.LineTo(rcEdge.right,rcEdge.bottom);
				}
				...
				if (GetBarStyle() & CBRS_ALIGN_LEFT)
				{
					//mdc.MoveTo(rcEdge.right,rcEdge.top);
					//mdc.LineTo(rcEdge.right,rcEdge.bottom);
					mdc.DrawEdge(rcEdge,EDGE_ETCHED,BF_LEFT);
				}
				if (GetBarStyle() & CBRS_ALIGN_TOP)
				{
					//mdc.MoveTo(rcEdge.right,rcEdge.top);
					//mdc.LineTo(rcEdge.right,rcEdge.bottom);
					mdc.DrawEdge(rcEdge,EDGE_ETCHED,BF_TOP);
				}
				if (GetBarStyle() & CBRS_ALIGN_RIGHT)
				{
					//mdc.MoveTo(rcEdge.right,rcEdge.top);
					//mdc.LineTo(rcEdge.right,rcEdge.bottom);
					mdc.DrawEdge(rcEdge,EDGE_ETCHED,BF_RIGHT);
				}
				else if (GetBarStyle() & CBRS_ALIGN_BOTTOM)
				{
					//mdc.MoveTo(rcEdge.left,rcEdge.bottom);
					//mdc.LineTo(rcEdge.right,rcEdge.bottom);
					mdc.DrawEdge(rcEdge,EDGE_ETCHED,BF_BOTTOM);
				}
			}
		}
    }
