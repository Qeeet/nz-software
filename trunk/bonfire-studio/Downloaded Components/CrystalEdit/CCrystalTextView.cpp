...
COLORREF CCrystalTextView::GetColor(int nColorIndex)
{
	...
	case COLORINDEX_BKGND:
		return pApp->GetAppColor(APP_COLOR_BACKGROUND);
	case COLORINDEX_NORMALTEXT:
		return pApp->GetAppColor(APP_COLOR_TEXT);
	case COLORINDEX_STRING:
		return pApp->GetAppColor(APP_COLOR_STRING);
	case COLORINDEX_COMMENT:
		return pApp->GetAppColor(APP_COLOR_COMMENT);
	case COLORINDEX_EL_TAG:
		return pApp->GetAppColor(APP_COLOR_ELEMENT);
	case COLORINDEX_EL_NAME:
		return pApp->GetAppColor(APP_COLOR_EL_NAME);
	case COLORINDEX_EL_ATTRIB:
		return pApp->GetAppColor(APP_COLOR_EL_ATTRIB);
	case COLORINDEX_SELBKGND:
		return pApp->GetAppColor(APP_COLOR_SEL_BACKGROUND);
	case COLORINDEX_SELTEXT:
		return pApp->GetAppColor(APP_COLOR_SEL_TEXT);
	case COLORINDEX_PREPROCESSOR:
		return pApp->GetAppColor(APP_COLOR_PREPROCESSOR);
	case COLORINDEX_KEYWORD:
		return pApp->GetAppColor(APP_COLOR_KEYWORD);
	case COLORINDEX_NUMBER:
		return pApp->GetAppColor(APP_COLOR_NUMBER);
	case COLORINDEX_ASP:
		return pApp->GetAppColor(APP_COLOR_ASP);
...
void CCrystalTextView::DrawMargin(CDC *pdc, const CRect &rect, int nLineIndex)
{
	...
	WORD waBmpData[8];
	waBmpData[0] = 170; waBmpData[1] = 85; waBmpData[2] = 170; waBmpData[3] = 85;
	waBmpData[4] = 170; waBmpData[5] = 85; waBmpData[6] = 170; waBmpData[7] = 85;

	CBitmap bmpDottedBmp; bmpDottedBmp.CreateBitmap(8,8,1,1,waBmpData); // create pattern
	CBrush brsDottedBrush(&bmpDottedBmp);

	COLORREF clrOldText = pdc->SetTextColor(GetSysColor(COLOR_BTNHIGHLIGHT));
	COLORREF clrOldBk = pdc->SetBkColor(GetSysColor(COLOR_BTNFACE));

	pdc->FillRect(rect,&brsDottedBrush); // new way

	pdc->SetTextColor(clrOldText);
	pdc->SetBkColor(clrOldBk);

	brsDottedBrush.DeleteObject();
	bmpDottedBmp.DeleteObject();

	//pdc->FillSolidRect(rect, GetColor(COLORINDEX_SELMARGIN)); // old way
...
void CCrystalTextView::ResetView()
{
	...
	if (::IsWindow(m_hWnd))
		UpdateCaret();
	m_bLastSearch = FALSE;
	m_bShowInactiveSelection = TRUE; // modified to always show selection
...
void CCrystalTextView::OnInitialUpdate()
{
	...
	if (pSplitter != NULL)
	{
		// hack for hiding source view
		if (GetDlgCtrlID() == AFX_IDW_PANE_FIRST + 255)
			return;
...
/*
static int RegExFindStringHelper(const CCRegEx& reExp, const TCHAR *str,
				   CCRegExMatch& xMatches, UINT nFlags = match_default, int index = 0)
{
	if (index >= 0)
    {
		if (index == 0 && str[index] != _T('\0'))
		{
			if (query_match(str, xMatches, reExp, nFlags))
				return 0;
			++index;
		}
		
		while (str[index] != _T('\0'))
		{
			if (query_match(str + index, xMatches, reExp, nFlags | match_not_bol))
				return index;
			++index;
		}
    }
