...
void CSizingControlBarG::NcCalcClient(LPRECT pRc, UINT nDockBarID)
{
    ...
    if (bHorz)
        rc.DeflateRect(m_cyGripper + 2, 0, 0, 0);
    else
    ...
    CPoint ptOrgBtn;
    if (bHorz)
        ptOrgBtn = CPoint(rc.left - 16, rc.top);
    else
        ptOrgBtn = CPoint(rc.right - 13, rc.top - 14);
...
void CSCBButton::Paint(CDC* pDC)
{
    ...
    pDC->TextOut(ptOrg.x + n + 1, ptOrg.y + n, CString(_T("r"))); // x-like
